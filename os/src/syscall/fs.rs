...
pub fn sys_fstat(fd: usize, st: *mut Stat) -> isize {
    ...
    if let Some(file) = &inner.fd_table[fd] {
        ...
        let st_virt_addr: VirtAddr = (st as usize).into();
        let buffers = translated_byte_buffer(
            current_user_token(),
            st_virt_addr.0 as *const u8,
            size_of::<Stat>(),
        );
        unsafe {
            let stat_bytes =
...
pub fn sys_linkat(old_name: *const u8, new_name: *const u8) -> isize {
    ...
    match linkat(old_name.as_str(), new_name.as_str()) {
        true => 0,
        false => -1,
    }
}
